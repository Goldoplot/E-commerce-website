...
function get_unique_categories(){
    ...
    // Displaying categories
            ...
            if($num_of_rows==0){
                echo "<h2 class='text-center text-danger'>No products are available in this category</h2>";
            }
            while ($row = mysqli_fetch_assoc($result_query)) {
                $product_id = $row['product_id'];
                $product_title = $row['product_title'];
...
function get_unique_brands(){
    ...
    if(isset($_GET['brand'])) {
        ...
        $num_of_rows=mysqli_num_rows($result_query);
        if($num_of_rows==0){
            echo "<h2 class='text-center text-danger'>No brand are available in this category</h2>";
        }
        while ($row = mysqli_fetch_assoc($result_query)) {
            $product_id = $row['product_id'];
            $product_title = $row['product_title'];
...
// get search product function
function search_product(){
    global $con;
    if(isset($_GET['search_data_product'])) {
        $search_data_value=$_GET['Search_data'];
        $search_query ="Select * from `products` where product_keywords or product_title like '%$search_data_value%'";
        $result_query = mysqli_query($con, $search_query);
        $num_of_rows=mysqli_num_rows($result_query);
        if($num_of_rows==0){
            echo "<h2 class='text-center text-danger'>No result match. No product for this category</h2>";
        }
        while ($row = mysqli_fetch_assoc($result_query)) {
            $product_id = $row['product_id'];
            $product_title = $row['product_title'];
    ...
    if(isset($_GET['product_id'])) {
        if(!isset($_GET['category'])){
            if(!isset($_GET['brand'])){
                ...
                <div class='row''>
                    <div class='col-md-12'>
                         <h4 class='text-center text-info mb-15'>Related Products</h4>
                    </div>
                    <div class='col-md-6'>
