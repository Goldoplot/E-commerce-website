...
if(isset($_GET['edit_product'])) {
    ...
    //fetching category id and name
    $select_category = "select * from `categories` where category_id=$category_id";
    $result_category = mysqli_query($con, $select_category);
    $row_category = mysqli_fetch_array($result_category);
    $category_title = $row_category['category_title'];

    //fetching brand id and name
...
<?php
 if(isset($_POST['edit_product'])) {
     ...
     //checking if fields or empty or not
     if($product_title=='' or $product_description=='' or $product_keywords=='' or $product_category=='' or $product_brand=='' or $product_image1=='' or $product_image2=='' or $product_image3=='' or $product_price=='' ){
         ...
         move_uploaded_file($temp_image3, "./product_images/$product_image3");
            ...
            //update query
         $update_product="update `products` set category_id=$product_category, brand_id='$product_brand', product_title='$product_title', product_description='$product_description',
                      product_keywords='$product_keywords', product_image1='$product_image1', product_image2='$product_image2', product_image3='$product_image3', product_price=$product_price, date=NOW() where product_id=$edit_id";
            $result_update=mysqli_query($con,$update_product);
            if($result_update){
                echo "<script>alert('Product updated successfully');
                window.open('./insert_product.php','_self')</script>";
            }
     }
}
 ?>